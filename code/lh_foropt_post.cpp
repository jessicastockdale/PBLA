...
/// Posterior function for PBLA Foot and Mouth, spatial beta ///
...
// [[Rcpp::depends(RcppGSL)]]
// [[Rcpp::export]]
 double log_post(const RcppGSL::Vector &params, const RcppGSL::Vector &R2, RcppGSL::Matrix &pmat, int N, int n, int m, const RcppGSL::Vector &n_c, const RcppGSL::Vector &n_s) {
  ...
  /// begin lh calculation
    ...
    /// add in priors
    lh = lh+ log(gsl_cdf_gamma_P(beta0, 0.001, 1/0.001)) + log(gsl_cdf_gamma_P(g, 0.001, 1/0.001)) +
      log(gsl_cdf_gamma_P(v, 1, 1/0.1)) + log(gsl_cdf_gamma_P(ep, 1, 1/0.001)) +
      log(gsl_cdf_gamma_P(xi, 1, 1/0.001)) + log(gsl_cdf_gamma_P(ze, 1, 1/0.001));
